...
class RemoveColumn(TransformerMixin):
    
    """
    Entfernt eine angegebene Spalte aus einem Dataframe.
    """
    ...
    def transform(self, X):
        ...
        X_removed = X.drop(columns=[self.column_name], errors='ignore')
        return X_removed
...
class RobustScalerTransformer(TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        # Führe die Skalierung auf den Daten durch und messe die Laufzeit
        start_time = time.time()
        self.scaler.fit(X)
        end_time = time.time()
        self.scaling_time = end_time - start_time
    ...
    def transform(self, X):
        ...
        print("Skalierung: Laufzeit der Skalierung: {:.4f} Sekunden".format(self.scaling_time+transformation_time))
        print("------------------------")
...
class CategoricalLabelEncoder(TransformerMixin):
    
    """
    Encodet die kategorischen Werte eines Dataframes mit dem LabelEncoder.
    """
    ...
    def transform(self, X):
        ...
        # Bestimmen der kategorischen Spalten
        cat_columns = X.columns
        label_encoders = {}
        encoded_features = 0
        
        # Initialisieren von LabelEncodern für jede kategorische Spalte
        for column in cat_columns:
            label_encoders[column] = LabelEncoder()
            # Fitten des LabelEncoders auf die Daten in der Spalte
            label_encoders[column].fit(X[column])
        # Kopieren des DataFrames, um Änderungen nicht direkt im Original zu machen
        X_encoded = X.copy()
        # Anwenden des Label-Encoding auf jede kategorische Spalte
        for column in cat_columns:
            X_encoded[column] = label_encoders[column].transform(X[column])
            encoded_features +=1
        # Berechnung der Anzahl der encodierten Merkmale
...
class DateTimePreprocessor(TransformerMixin):
    ...
    def transform(self, X):
        ...
        for col in datetime_columns:
            ...
            X_cleaned[col] = (X_cleaned[col] - datetime(1970, 1, 1)).dt.total_seconds()
