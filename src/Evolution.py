...
class Evolution:
    ...
    @staticmethod
    def select_instances(n_individual, baseline_individual, minimum_sample_size=1000): #Auswahl von Instanzen, um die Trainingsdaten zu reduzieren
        ...
        sample_size = int(baseline_individual.X_train.shape[0] * 0.80)  # Start mit 90% der Daten
        ...
        while improving and sample_size > minimum_sample_size:
            ...
            sample_size = int(sample_size * 0.8)  # Sanftere Reduktion

        end_full = time.time()
        meta_model_time = end_full - start_full
        print(f"Suchzeit Meta-Modell: {meta_model_time}")
        return meta_model_time, best, best_coef, selected_instances, baseline_full_data # ausgesuchten besten Instanzen zurückgeben und die Fitness der ursprünglichen Trainingsdaten

    @staticmethod
    def evaluate_selected_instances(individual, baseline_full_data, baseline_individual, GA_representation, original_fitness_time, plot=False): #Vergleich der Metrik auf dem vollständigen Datensatz mit der Approximation
    ...
    def CHCqx(baseline_individual, f, n_individual, f_no_change, population_size=50, verbose=1):
        ...
        start = time.time()
        meta_model_time, best, best_coef, selected_instances, baseline_full_data = Evolution.select_instances(n_individual, baseline_individual)
        
        meta_dict = {
        "meta_model_time": meta_model_time,
        "f_s(g)": best,
        "coef": best_coef,
        "sample_size": len(selected_instances),
        "full_sample_size": baseline_individual.X_train.shape[0]
        }
        print(f"Meta-Modell Bewertung:\n")
        print(f"f_s(g): {meta_dict['f_s(g)']} = (1 - {meta_dict['coef']}) + {meta_dict['sample_size']}/{meta_dict['full_sample_size']}")
        print(f"Berechnungszeit: {meta_dict['meta_model_time']} Sekunden")
        ...
        while (no_change < f_no_change):
            no_change += 1
            log_df, population, d, FS_time = Evolution.CHC(gaqx_individual, toolbox, d, population, max_generations=f, verbose=0)
            fitness_sum = 0
            ind_id = 0
            generation+=f
        ...
        return meta_dict, gaqx_log_df, baseline_full_data
    ...
    def CHC(dataset, toolbox, d, population=False, population_size=40, divergence=3,
        ...
        FS_time = end - start
        return logDF, population, d, FS_time
    ...
    @staticmethod
    def PSO(baseline_individual, options, n_particles, timeout=np.inf, steps=np.inf, topology=Star(), steps_no_change=10, verbose=1):
        start = time.time()
        gaqx_log_df = pd.DataFrame(columns=['ind', 'time', 'fitness'])
